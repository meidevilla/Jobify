...
def dashboard(request):
    ...
    monthly_applications = (applications.annotate(month=ExtractMonth('date_applied'), year=ExtractYear('date_applied')).values('month', 'year').annotate(total=Count('id')).order_by('year', 'month'))
...
def calendar(request):
    ...
    for job in job_applications:
        interview_data.append({
            ...
            'start': job.interview_schedule.strftime('%Y-%m-%dT%H:%M:%S'),
            'end': (job.interview_schedule + timedelta(hours=1)).strftime('%Y-%m-%dT%H:%M:%S'),
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print("UserProfileForm Errors:", form.errors)
        if form2.is_valid():
            form2.save()
        else:
            print("UserChangeForm Errors:", form2.errors)
